...
DROP TABLE IF EXISTS out_sppimp_10km_teeb_s1_2050_cons_crenvu;
CREATE TABLE out_sppimp_10km_teeb_s1_2050_cons_crenvu AS 
...
AND (st.code) in ('CR','EN','VU'); 
; 
--view subset of result to check it worked - 
-- SELECT * FROM out_sppimp_10km_teeb_s1_2050_cons_crenvu ORDER BY cell_sp LIMIT 1000;


--see how many species have values 
-- SELECT COUNT(1) from (select /*species as species*/ id_no from out_sppimp_10km_teeb_s1_2050_cons_crenvu group by id_no/*species*/) as foo;


--should be same as the count on the input table out_spp_calc_areacells_10km_teeb_s1_2050_cons_crenvu
-- SELECT count(1) from (select /*species as species*/ id_no from out_spp_calc_areacells_10km_teeb_s1_2050_cons_crenvu group by id_no/*species*/) as foo;

 
--see if null rows are null in all columns
-- SELECT * FROM out_sppimp_10km_teeb_s1_2050_cons_crenvu WHERE sppimp_eq IS NULL ORDER BY cell_sp;


--make final importance values for watersheds by grouping by watershed ids
DROP TABLE IF EXISTS out_cell_imp_10km_teeb_s1_2050_cons_crenvu;
CREATE TABLE out_cell_imp_10km_teeb_s1_2050_cons_crenvu AS
SELECT o.cell_id, SUM(o.sppimp_eq) AS cellimp_eq
--,SUM(o.sppimp_max) AS cellimp_max, SUM(o.sppimp_min) AS cellimp_min 
FROM out_sppimp_10km_teeb_s1_2050_cons_crenvu as o
GROUP BY o.cell_id;


--join final results to watershed polygons for viewing
DROP TABLE IF EXISTS out_cell_imp_shape_10km_teeb_s1_2050_cons_crenvu;
CREATE TABLE out_cell_imp_shape_10km_teeb_s1_2050_cons_crenvu AS 
SELECT o.*, p.the_geom as the_geom  
FROM 
out_cell_imp_10km_teeb_s1_2050_cons_crenvu AS o,
cells_10km_teeb AS p 
WHERE o.cell_id=p.cell_id;

--convert to wgs84 for viewing purposes
ALTER TABLE out_cell_imp_shape_10km_teeb_s1_2050_cons_crenvu
...
DROP TABLE IF EXISTS out_spcell_change_10km_teeb_s1_2050_cons_crenvu; 
CREATE TABLE out_spcell_change_10km_teeb_s1_2050_cons_crenvu AS
SELECT 
...
--select *  from out_spcell_change_10km_teeb_s1_2050_cons_crenvu order by sp_cell_chg desc; --habt_chg desc;
...
DROP TABLE  IF EXISTS out_cell_change_10km_teeb_s1_2050_cons_crenvu;
CREATE TABLE out_cell_change_10km_teeb_s1_2050_cons_crenvu AS
select a.cell_id, sum(a.sp_cell_chg) 
as cell_change
from out_spcell_change_10km_teeb_s1_2050_cons_crenvu as a
group by a.cell_id 
order by cell_change desc;


-- make a table linking polygons to the results
DROP TABLE  IF EXISTS out_cell_change_shape_10km_teeb_s1_2050_cons_crenvu;
CREATE TABLE out_cell_change_shape_10km_teeb_s1_2050_cons_crenvu  AS 
SELECT o.*, p.the_geom as the_geom  
FROM out_cell_change_10km_teeb_s1_2050_cons_crenvu AS o, cells_10km_teeb AS p 
WHERE o.cell_id=p.cell_id;


--convert to wgs84 for viewing purposes
ALTER TABLE out_cell_change_shape_10km_teeb_s1_2050_cons_crenvu
 ALTER COLUMN the_geom TYPE geometry(MultiPolygon,4326) 
  USING ST_Transform(the_geom,4326);
 
update out_cell_change_shape_10km_teeb_s1_2050_cons_crenvu
...
DROP TABLE  IF EXISTS out_cell_loss_10km_teeb_s1_2050_cons_crenvu;
CREATE TABLE out_cell_loss_10km_teeb_s1_2050_cons_crenvu AS
select a.cell_id, sum(a.sp_cell_chg) 
as cell_loss
from
(
...
from out_spcell_change_10km_teeb_s1_2050_cons_crenvu
)as a
group by a.cell_id;


-- make a table linking polygons to the results
DROP TABLE  IF EXISTS out_cell_loss_shape_10km_teeb_s1_2050_cons_crenvu;
CREATE TABLE out_cell_loss_shape_10km_teeb_s1_2050_cons_crenvu  AS 
SELECT o.*, p.the_geom as the_geom  
FROM out_cell_loss_10km_teeb_s1_2050_cons_crenvu AS o, cells_10km_teeb AS p 
WHERE o.cell_id=p.cell_id;


--convert to wgs84 for viewing purposes
ALTER TABLE out_cell_loss_shape_10km_teeb_s1_2050_cons_crenvu
 ALTER COLUMN the_geom TYPE geometry(MultiPolygon,4326) 
  USING ST_Transform(the_geom,4326);
 
update out_cell_loss_shape_10km_teeb_s1_2050_cons_crenvu
set cell_loss = coalesce(cell_loss, 0);

-----------------------------------------------------------------------------------------------------------
----------------------code for exporting shapefiles - just for normalised crenvu currently 
/*

--For exporting using ogr2ogr (osgeo4w command line) into separate shapefiles for change maps

ogr2ogr --config FGDB_BULK_LOAD YES  -progress -f "ESRI Shapefile" -sql "SELECT * FROM teeb_2017.out_cell_loss_shape_10km_teeb_s1_2050_cons_crenvu" C:\Data\final_bd_results\teeb_2017\1km_lshift\cons PG:"host=localhost user=postgres password=Seltaeb1 dbname=biodiv_processing" -nln out_cell_loss_shape_10km_teeb_s1_2050_cons_crenvu -nlt POLYGON -lco "SHPT=POLYGON"  -a_srs "EPSG:4326"

ogr2ogr --config FGDB_BULK_LOAD YES  -progress -f "ESRI Shapefile" -sql "SELECT * FROM teeb_2017.out_cell_change_shape_10km_teeb_s1_2050_cons_crenvu" C:\Data\final_bd_results\teeb_2017\1km_lshift\cons PG:"host=localhost user=postgres password=Seltaeb1 dbname=biodiv_processing" -nln out_cell_change_shape_10km_teeb_s1_2050_cons_crenvu -nlt POLYGON -lco "SHPT=POLYGON"  -a_srs "EPSG:4326"

ogr2ogr --config FGDB_BULK_LOAD YES  -progress -f "ESRI Shapefile" -sql "SELECT * FROM teeb_2017.out_cell_imp_shape_10km_teeb_s1_2050_cons_crenvu" C:\Data\final_bd_results\teeb_2017\1km_lshift\cons PG:"host=localhost user=postgres password=Seltaeb1 dbname=biodiv_processing" -nln out_cell_imp_shape_10km_teeb_s1_2050_cons_crenvu -nlt POLYGON -lco "SHPT=POLYGON"  -a_srs "EPSG:4326"

ogr2ogr --config FGDB_BULK_LOAD YES  -progress -f "ESRI Shapefile" -sql "SELECT * FROM teeb_2017.out_cell_imp_shape_10km_teeb_baseline_cons_crenvu" C:\Data\final_bd_results\teeb_2017\1km_lshift\cons PG:"host=localhost user=postgres password=Seltaeb1 dbname=biodiv_processing" -nln out_cell_imp_shape_10km_teeb_baseline_cons_crenvu -nlt POLYGON -lco "SHPT=POLYGON"  -a_srs "EPSG:4326"
